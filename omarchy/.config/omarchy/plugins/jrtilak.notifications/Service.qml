...
Item {
  ...
  // Match the previous SwayNC layout: toasts are fixed to the bottom-right.
  // They clear a bottom or right bar without moving away from that corner.
  ...
  // Match the previous SwayNC behavior: normal notifications use the standard
  // message sound, critical alerts use the warning sound, and low-priority
  // notifications remain silent. DND-silenced notifications return before
  // this function is called.
  function playNotificationSound(urgency) {
    if (urgency === NotificationUrgency.Low) return
    var eventName = urgency === NotificationUrgency.Critical
      ? "dialog-warning"
      : "message-new-instant"
    Quickshell.execDetached(["canberra-gtk-play", "-i", eventName, "-d", "omarchy-notifications"])
  }
