...
This is `jrtilak.notification-center`, a user-owned fork of
[`jankeesvw/omarchy-notification-center`](https://github.com/jankeesvw/omarchy-notification-center).
It preserves the upstream archive and interaction features while using
Omarchy's shared popup palette, border, radius, spacing, and control-state
tokens. It also adds an unread-only view and keyboard row navigation.
...
- **All / Unread** filters the current archive without deleting anything. Use
  `U` to toggle it from the keyboard.
- **Keyboard navigation** uses the arrow keys or `j`/`k`; Enter opens the
  selected notification and `x` dismisses it.
...
```bash
omarchy plugin remove jrtilak.notification-center
...
```bash
omarchy-shell jrtilak.notification-center toggle
```
