...
Item {
  ...
  BorderSurface {
    ...
    rightPadding: Style.spacing.xxl
    bottomPadding: Style.spacing.xl
    leftPadding: Style.spacing.xxl
    implicitHeight: Math.max(texts.implicitHeight, avatar.height)
      + contentTopInset + contentBottomInset

    // Use Omarchy's shared control states instead of hardcoded fills and
    // radii. Cards now follow every theme's density, corner radius, border
    // color/gradient, and hover/selected treatment.
    color: root.selected
      ? Style.selectedFillFor(root.foreground, Color.accent, Color.urgent)
      : Style.controlFill(false, root.hovered, root.foreground, Color.accent)
    borderSpec: Border.controlSpec(
      root.selected ? "selected" : (root.hovered ? "hover-cursor" : "normal"),
    ...
    Rectangle {
      ...
      anchors.leftMargin: card.borderLeft + Style.spacing.sm
      anchors.topMargin: card.borderTop + Style.spacing.sm
      anchors.bottomMargin: card.borderBottom + Style.spacing.sm
      width: Style.space(3)
      radius: width / 2
      color: Color.urgent
    }
    ...
    Column {
      ...
      Item {
        ...
        Rectangle {
          ...
          radius: Math.max(0, card.radius - Style.spacing.sm)
          color: "black"
          visible: false
          layer.enabled: true
          layer.smooth: true
