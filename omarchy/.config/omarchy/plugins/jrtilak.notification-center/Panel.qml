...
// User-owned fork of jankeesvw.notification-center.
// A complete notification center for Omarchy: everything you were sent,
// still there when you go back for it, styled through the active shell theme.
//
...
Panel {
  ...
  moduleName: "jrtilak.notification-center"
  ipcTarget: "jrtilak.notification-center"

  readonly property string omarchyPath: Quickshell.env("OMARCHY_PATH")

  // Panel content follows the popup palette, which a theme may intentionally
  // make different from the bar palette.
  readonly property color foreground: Color.popups.text
  readonly property string fontFamily: Style.font.family

  // ----------------------------------------------------------------- settings

  readonly property int panelWidth: setting("panelWidth", 480)
  ...
  function bindStore() {
    ...
    var s = host.serviceFor("jrtilak.notification-center")
    if (!s) return
    store = s
    pushSettings()
    rebuild()
  ...
  readonly property int newAtOpen: {
    var count = 0
    for (var i = 0; i < entries.length; i++)
      if (Number(entries[i].timestamp || 0) > readMark) count++
    return count
  }
  ...
  function moveSelection(step) {
    if (step === 0 || rows.count === 0) return
    if (currentIndex < 0) currentIndex = step > 0 ? 0 : rows.count - 1
    else currentIndex = Math.max(0, Math.min(rows.count - 1, currentIndex + step))
    list.positionViewAtIndex(currentIndex, ListView.Contain)
  }

  function activateSelection() {
    if (currentIndex < 0 || currentIndex >= rows.count) return
    activate(rows.get(currentIndex))
  }

  function removeSelection() {
    if (currentIndex < 0 || currentIndex >= rows.count) return
    var index = currentIndex
    remove(rows.get(index).key)
    Qt.callLater(function() {
      currentIndex = rows.count > 0 ? Math.min(index, rows.count - 1) : -1
  ...
  function matches(entry) {
    if (unreadOnly && Number(entry.timestamp || 0) <= readMark) return false
  ...
  function rebuild() {
    ...
    if (currentIndex >= rows.count) currentIndex = rows.count - 1
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onDeleteRequested: root.removeSelection()
      ...
      onTextKey: function(text) {
        ...
        else if (text === "u" || text === "U") root.unreadOnly = !root.unreadOnly
      }
      ...
      Column {
        ...
        Item {
          id: header
          width: parent.width
          height: Math.max(heading.implicitHeight, actions.height)
          ...
          Column {
            ...
            Text {
              textFormat: Text.PlainText
              text: {
                if (!root.loaded) return "Loading archive"
                if (root.newAtOpen > 0)
                  return root.entries.length + " kept · " + root.newAtOpen + " new"
                return root.entries.length === 1 ? "1 notification kept"
                  : root.entries.length + " notifications kept"
              }
              font.family: root.fontFamily
          ...
          Row {
            ...
            Button {
              ...
              tooltipText: "Toggle unread notifications  ( U )"
        ...
        ListView {
          ...
          delegate: NotificationRow {
            ...
            width: list.width - list.lane
            selected: root.currentIndex === row.index
            app: model.app
        ...
        Text {
          ...
          text: !root.loaded ? "Reading the archive\u2026"
              ...
              : root.unreadOnly ? "No unread notifications"
        ...
        Text {
          ...
          text: (root.filter !== "" || root.unreadOnly)
            ? rows.count + " shown \u00b7 " + root.entries.length + " kept"
            : root.entries.length === 1
              ? "1 notification kept \u00b7 " + root.keepDays + " days"
